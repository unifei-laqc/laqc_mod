...
def usar_arq_conf():
    """
    Questiona o usuário se irá, usar ou não, <arquivo>.conf.

    Returns
    -------
    Boolean
        True, usará <arquivo>.conf a ser informado
        False, não usará nenhum arquivo de configuração
    """
...
def ler_arq_conf():
    ...
    """
    # Dicionário contendo as modificações a serem realizadas
    alterar = {}
                ...
                with open(arquivo_conf, "r") as f_arquivo_conf:
                    for line in f_arquivo_conf:
                        ...
                        if (line[0] != ";") and (len(line.strip()) > 0):
                            ...
                            alterar[variavel.strip()] = valor.strip()
        ...
        if vlr_memoria.upper() == "SAIR":
            ...
            if vlr_memoria == "":
                ...
                try:
                    ...
                    print(f" + Valor informado ({vlr_memoria}), não é um número inteiro.")
                    questao_ok = False
...
def layers_camadas(configuracoes):
    ...
    if vlr_simbolo.upper() == "SAIR":
        ...
        if vlr_simbolo == "":
            ...
            if vlr_simbolo in ["S", "s", "N", "n"]:
                configuracoes['layers_simbolo'] = str(vlr_simbolo in ["S", "s"])
            else:
                print(f" + Valor informado ({vlr_simbolo}), inválido! Considerando [N]")
                configuracoes['layers_simbolo'] = "False"

    vlr_layer_high = input(" - Linhas para 'layer high'? [0-0]".ljust(TAM_TEXTO, ".")
                           + ": ").strip()
    if vlr_layer_high.upper() == "SAIR":
        tchau()
    else:
        if vlr_layer_high == "":
    ...
    vlr_layer_middle = input(" - Linhas para 'layer middle'? [0-0]".ljust(TAM_TEXTO, ".")
                             + ": ").strip()
    if vlr_layer_middle.upper() == "SAIR":
        tchau()
    else:
        if vlr_layer_middle == "":
...
if __name__ == "__main__":
    ...
    # Dicionário com as alterações a serem realizadas
    dict_alteracoes = {}

    # Questiona sobre o uso do arquivo de configuração
    if usar_arq_conf():
